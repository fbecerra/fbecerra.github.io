Promise.all([
    d3.csv("estandar_simce_mate_slep.csv")
]).then(function(datos) {
    ...
    const regions = Array.from(new Set(datos[0].filter(d => d.estandar === 'Total').map(d => d.slep)));
    ...
    const plotTerritory = (data, col) =>{
        ...
        regions.forEach(reg => {
            ...
            if (reg !== 'TODAS') {
                const thisData = datum.filter(d => d.data.slep === reg);
                g.selectAll("ring")
                    .data(thisData)
                    .join("circle")
                    .attr("class", "ring")
    ...
    const nacional = regiones.filter(d => d.slep === 'TODAS' && indicadores.includes(d.estandar));

    regions.forEach(reg => {
        const thisRegion = regiones.filter(d => d.slep === reg && indicadores.includes(d.estandar));
