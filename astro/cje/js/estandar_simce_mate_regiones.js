Promise.all([
    d3.csv("estandar_simce_mate_regiones.csv")
]).then(function(datos) {
    ...
    const regions = Array.from(new Set(datos[0].filter(d => d.estandar === 'Total').map(d => d.region)));
    ...
    const nacional = regiones.filter(d => d.region === 'TODAS' && indicadores.includes(d.estandar));

    regions.forEach(reg => {
        const thisRegion = regiones.filter(d => d.region === reg && indicadores.includes(d.estandar));
        
        filters.slice(5, 10).forEach(filter => {
