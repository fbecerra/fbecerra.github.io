Promise.all([
    d3.csv("repitencia_regiones.csv")
]).then(function(datos) {

    const indicadores = ['Repite'];
    ...
    indicadores.forEach(indicador => {
        ...
        const regions = regiones.map(d => d.region);
        ...
        const plotTerritory = (data, col) =>{
            ...
            const datum = beeswarm(data.filter(d => d.region !== 'TODAS'));
            const nacional = data.filter(d => d.region === 'TODAS');
            ...
            regions.forEach(reg => {
                ...
                g.selectAll(".dot")
                    ...
                    .join("circle")
                    .attr("class", 'dot')
                    .attr("fill", d => d.data.region === reg ? color : "#D9D9D0")
                    .attr("cx", d => d.x)
                    .attr("cy", d => d.y)
                    .attr("r", r);

                if (reg !== 'TODAS') {
                    const thisData = datum.filter(d => d.data.region === reg);
                    g.selectAll("ring")
                        .data(thisData)
                        .join("circle")
                        .attr("class", "ring")
                ...
                if (dataToPlot.length > 7) {
                    ...
                    if (reg !== 'TODAS') {
                        ...
                        div.append("div")
                            .style("width", "250px")
                            .style('display', 'inline-block')
                            .html(`La region de ${reg} y filtro ${col} tiene una repitencia de ${regionValue}%, lo que esta ${diffRegional === 0 ? "justo en el" : Math.abs(diffRegional).toFixed(2) + '%'} 
        ...
        filters.forEach(filter => {
            ...
            regions.forEach(reg => {
                const thisRegion = regiones.filter(d => d.region === reg)[0];
                ...
                const nacional = regiones.filter(d => d.region === 'TODAS').map(d => {
                    return {
                        value: d[filter+'_tot']
                    }
                });
